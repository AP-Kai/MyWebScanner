...
public class StringSubSame{
    ...
    public String intersection(String a, String b) {
        ...
        //否则取 a的第i-1个b的j个与a的i个b的j-1个中最大的。
        // 因为对比a[i-1][j-1],要不是新增的a[i]个字符与前b[j-1]的字符产生了新的匹配，要不是增加的b[j]个字符与a[i-1]个字符产生了新的匹配。
        // 如果新增的a[i]和b[j]都贡献了，（如字符串ad和da，在i=1，j=1时，a[1]=d与下面的字符产生了匹配，同时b[1]=a与前者产生匹配）。
        // 因为是序列，新增的匹配要作为最后一位，所以只能任选一个作为新增的匹配。
